...
class cosmicsimage:
    ...
    def clean(self, mask = None, verbose = None):
        ...
        if np.size(self.satstars) > 0:#self.satstars != None:
...
def tofits(outfilename, pixelarray, hdr = None, verbose = True):
    ...
    if hdr == None: # then a minimal header will be created
        ...
        fits.writeto(outfilename,pixelarray.transpose(),overwrite=True)
    else: # this if else is probably not needed but anyway ...
    #       hdu = pyfits.PrimaryHDU(pixelarray.transpose(), hdr)
        fits.writeto(outfilename,pixelarray.transpose(),header=hdr,overwrite=True)
...
def rebin(a, newshape):
    ...
    shape = a.shape
    lenShape = len(shape)
    factor = np.asarray(shape//newshape,dtype=int)#np.asarray(newshape)
    #print factor
    evList = ['a.reshape('] + \
